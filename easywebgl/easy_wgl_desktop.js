...
const ewgl_desktop = 
{
	...
	console:
	{
		...
		toggle: function(c)
		{
			...
			if (c !== this.internal_on_off)
			{
				this.internal_on_off = c;
				if (c)
				{
					ewgl_desktop.cons_div_elt.style.bottom="0";
	//				ewgl_desktop.cons_div_elt.style.visbility = "visible";
				}
				else
				{
					let h = -1 * (ewgl_desktop.cons_elt.offsetHeight + 5);
					ewgl_desktop.cons_div_elt.style.bottom= h+"px";
					//ewgl_desktop.cons_div_elt.style.visbility = "hidden";
				}
			}
...
//				this.custom("<par style='color:"+this.color_def+"'> UNDEFINED </par>");
	...
	cons_div_elt : document.getElementById("console_div"),
	...
	launch_desktop: function()
	{
		...
		setTimeout(()=>{ewgl_desktop.console.toggle(false);},3000);
	},
}
...
const UserInterface = 
{
	...
	toggle: function(i)
	{
		...
		if (ewgl_common.interface_on_off !== i)
		{
			ewgl_common.interface_on_off = i;
			if (i)
			{
				ewgl_desktop.interf_elt.style.left="0px";
//				ewgl_desktop.interf_elt.style.visbility = "visible";
			}
			else
			{
				let w = -1 * (ewgl_desktop.interf_elt.offsetWidth + 5);
				ewgl_desktop.interf_elt.style.left= w+"px";
				//ewgl_desktop.interf_elt.style.visbility = "hidden";
			}
		}
	},
...
//				comment: true,
	...
	add_text_input: function(text, func)
	{
...
let Camera_ops = 
{
	...
	{
		const d = this.zcam*this.s_radius - this.frame.data[14];
		this.znear = Math.max( 0.01,d-this.s_radius);
...
function FileDroppedOnCanevas(func)
{
	...
	ewgl_desktop.canv_cons_elt.addEventListener("drop", e =>
	{
		...
		//	for (let f of files) { func.apply(null,files);}
...
function internal_update_wgl()
{
	requestAnimationFrame( (ts) => 
	{
		...
		if ((ewgl_desktop.update_needed || ewgl_desktop.continuous_update))
		{
			...
			{
				UserInterface.fps_txt.value = ewgl_desktop.fps.toFixed(4) + ' fps';
			}
